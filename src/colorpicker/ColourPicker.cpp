...
{
	this->DrawItem2(lpDrawItemStruct);
}

void CColourPicker::DrawItem1(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
...
void CColourPicker::DrawItem2(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
    ASSERT(lpDrawItemStruct);
...
// COMMENT: {6/7/2006 5:55:41 PM}    // Draw arrow
// COMMENT: {6/7/2006 5:55:41 PM}    if (m_bActive) state |= ODS_SELECTED;
// COMMENT: {6/7/2006 5:55:41 PM}    pDC->DrawFrameControl(&m_ArrowRect, DFC_SCROLL, DFCS_SCROLLDOWN  | 
// COMMENT: {6/7/2006 5:55:41 PM}                          ((state & ODS_SELECTED) ? DFCS_PUSHED : 0) |
// COMMENT: {6/7/2006 5:55:41 PM}                          ((state & ODS_DISABLED) ? DFCS_INACTIVE : 0));

// COMMENT: {6/7/2006 5:28:39 PM}    pDC->DrawFrameControl(&m_ArrowRect, DFC_SCROLL, DFCS_SCROLLDOWN  | 
// COMMENT: {6/7/2006 5:28:39 PM}                          ((state & ODS_SELECTED) ? DFCS_PUSHED : 0) |
// COMMENT: {6/7/2006 5:28:39 PM}                          ((state & ODS_DISABLED) ? DFCS_INACTIVE : 0));


    pDC->DrawFrameControl(&rect, DFC_BUTTON, DFCS_BUTTONPUSH);


	CRect left = lpDrawItemStruct->rcItem;
	left.DeflateRect(0, 0, 15, 0);
    pDC->DrawFrameControl(&left, DFC_BUTTON, DFCS_BUTTONPUSH);


	CRect right = lpDrawItemStruct->rcItem;
	right.DeflateRect(rect.Width() - 15, 0, 0, 0);
// COMMENT: {6/7/2006 6:27:58 PM}    pDC->DrawFrameControl(&right, DFC_BUTTON, DFCS_BUTTONPUSH);


	CRect color(left);
    ///color.DeflateRect(3, 3, 3, 3);
    color.DeflateRect(4, 4, 4, 4);
...
// COMMENT: {6/7/2006 6:33:28 PM}	pDC->SelectStockObject(NULL_PEN);
    pDC->Rectangle(color);
    pDC->SelectObject(pOldBrush);
	//{{
...
// COMMENT: {6/7/2006 5:56:09 PM}    // Draw arrow
// COMMENT: {6/7/2006 5:56:09 PM}    if (m_bActive) state |= ODS_SELECTED;
// COMMENT: {6/7/2006 5:56:09 PM}    pDC->DrawFrameControl(&m_ArrowRect, DFC_SCROLL, DFCS_SCROLLDOWN  | 
// COMMENT: {6/7/2006 5:56:09 PM}                          ((state & ODS_SELECTED) ? DFCS_PUSHED : 0) |
// COMMENT: {6/7/2006 5:56:09 PM}                          ((state & ODS_DISABLED) ? DFCS_INACTIVE : 0));


    // Draw arrow
// COMMENT: {6/7/2006 6:05:46 PM}    if (m_bActive) state |= ODS_SELECTED;
    pDC->DrawFrameControl(&right, DFC_SCROLL, DFCS_SCROLLDOWN  | 
                          ((state & ODS_SELECTED) ? DFCS_PUSHED : 0) |
                          ((state & ODS_DISABLED) ? DFCS_INACTIVE : 0));
    ...
    if (state & ODS_FOCUS) 
    {
        left.DeflateRect(2, 2, 3, 3);
        pDC->DrawFocusRect(left);
    }
}
