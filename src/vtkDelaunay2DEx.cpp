...
#define vtkDelaunay2D vtkDelaunay2DEx

vtkCxxRevisionMacro(vtkDelaunay2DEx, "$Revision: 1.73 $");
vtkStandardNewMacro(vtkDelaunay2DEx);
vtkCxxSetObjectMacro(vtkDelaunay2DEx,Transform,vtkAbstractTransform);
  ...
  for ( polys->InitTraversal(); polys->GetNextCell(npts,pts); )
    ...
    for (i=0; i<npts; i++)
      ...
      if ( ! this->Mesh->IsEdge(p1,p2) )
		...
		this->BadPoints.push_back(std::pair<vtkIdType, vtkIdType>(p1, p2));
		//}}
        }
...
//----------------------------------------------------------------------------
bool vtkDelaunay2D::IsOK()
{
	return this->BadPoints.empty();
}

const std::list< std::pair<vtkIdType, vtkIdType> >& vtkDelaunay2D::GetBadPoints()const
{
