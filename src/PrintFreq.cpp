...
void CPrintFreq::Insert(CTreeCtrl* pTreeCtrl, HTREEITEM htiPrintFreq)
{
	...
	COLLECT_TIMES_MACRO(this->print_restart);
	...
	for (; s != times.end(); ++s)
	{
		...
		this->InsertCtimeSeries(pTreeCtrl, hTime, this->print_end_of_period,     *s, "end_of_period_default ");
		...
		this->InsertCtimeSeries(pTreeCtrl, hTime, this->print_restart,           *s, "restart_file          ");
...
std::ostream& operator<< (std::ostream &os, const CPrintFreq& pf)
{
	...
	COLLECT_TIMES_MACRO(pf.print_restart);
	...
	for (; s != times.end(); ++s)
	{
		...
		pf.OutputCtimeSeries(os, pf.print_bc,                *s, "boundary_conditions   ");
		pf.OutputCtimeSeries(os, pf.print_comp,              *s, "components            ");
		pf.OutputCtimeSeries(os, pf.print_conductances,      *s, "conductances          ");
		pf.OutputCtimeSeries(os, pf.print_end_of_period,     *s, "end_of_period_default ");
		...
		pf.OutputCtimeSeries(os, pf.print_restart,           *s, "restart_file          ");
...
void CPrintFreq::Serialize(bool bStoring, hid_t loc_id)
{
	...
	static const char szPRINT_END_OF_PERIOD[]     = "print_end_of_period";
	...
	static const char szPRINT_RESTART[]           = "print_restart";
	...
	hid_t print_end_of_period_id     = 0;
	...
	if (bStoring)
	{
		...
		if (print_frequency_id > 0)
		{
			...
			print_zone_budget_id = ::H5Gcreate(print_frequency_id, szPRINT_ZONE_BUDGET, 0);
			ASSERT(print_zone_budget_id > 0);

			// Create the szPRINT_ZONE_BUDGET_HEADS group
			print_zone_budget_heads_id = ::H5Gcreate(print_frequency_id, szPRINT_ZONE_BUDGET_HEADS, 0);
			ASSERT(print_zone_budget_heads_id > 0);
	...
	else
	{
		...
		if (print_frequency_id > 0)
		{
			...
			print_zone_budget_id = ::H5Gopen(print_frequency_id, szPRINT_ZONE_BUDGET);

			// Open the szPRINT_ZONE_BUDGET_HEADS group
			print_zone_budget_heads_id = ::H5Gopen(print_frequency_id, szPRINT_ZONE_BUDGET_HEADS);
	...
	if (print_frequency_id > 0)
	{
		...
		if (print_zone_budget_id > 0) {
			// serialize print_zone_budget
			this->print_zone_budget.Serialize(bStoring, print_zone_budget_id);
			status = ::H5Gclose(print_zone_budget_id);
			ASSERT(status >= 0);
		}
		...
		if (print_zone_budget_tsv_id > 0) {
			// serialize print_zone_budget_tsv
			this->print_zone_budget_tsv.Serialize(bStoring, print_zone_budget_tsv_id);
...
void CPrintFreq::SyncWithSrcInput(void)
{
	...
	this->print_end_of_period.Append(::print_end_of_period);
	...
	this->print_restart.Append(::print_restart);
...
void CPrintFreq::InitSync(CPhastInput* input/*= NULL*/)
{
	...
	try
	{
		...
		if (bPhastInputIn || pInput)
		{
			...
			this->print_end_of_period[zero]     = ::current_print_end_of_period;
			...
			this->print_restart[zero]           = ::current_print_restart;
			this->save_final_heads              = ::save_final_heads;
